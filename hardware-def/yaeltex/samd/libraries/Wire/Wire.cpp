...
uint8_t TwoWire::requestFrom(uint8_t address, size_t quantity, bool stopBit, void *dataDestination)
{
  if(quantity == 0)
  {
    return 0;
  ...
  if(sercom->startTransmissionWIRE(address, WIRE_READ_FLAG))
  {
    if(!dataDestination){
      ...
      uint8_t *auxBuffer = (uint8_t*) dataDestination;
      // Read first data
      auxBuffer[0] = sercom->readDataWIRE();
      ...
      for (byteRead = 1; byteRead < quantity; ++byteRead)
      {
        sercom->prepareAckBitWIRE();                          // Prepare Acknowledge
        sercom->prepareCommandBitsWire(WIRE_MASTER_ACT_READ); // Prepare the ACK command for the slave
        auxBuffer[byteRead] = sercom->readDataWIRE();          // Read data and send the ACK
      }
    }
...
uint8_t TwoWire::requestFrom(uint8_t address, size_t quantity, void *dataDestination)
{
  return requestFrom(address, quantity, true, dataDestination);
}
