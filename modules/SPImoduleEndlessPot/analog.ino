...
int decodeInfinitePot(uint8_t i)
{
  ...
  ****************************************************************************/
  ...
  if (rotary[i].valueA > (rotary[i].previousValueA + controlRegister[MAPPING::ANALOG_HYSTERESIS]))       // check if new reading is higher (by <debounce value>), if so...
  {
    rotary[i].dirA = 1;                                              // ...direction of tap A is up
  }
  else if (rotary[i].valueA < (rotary[i].previousValueA - controlRegister[MAPPING::ANALOG_HYSTERESIS]))  // check if new reading is lower (by <debounce value>), if so...
  {
  ...
  if (rotary[i].valueB > (rotary[i].previousValueB + controlRegister[MAPPING::ANALOG_HYSTERESIS]))       // check if new reading is higher (by <debounce value>), if so...
  {
    rotary[i].dirB = 1;                                              // ...direction of tap B is up
  }
  else if (rotary[i].valueB < (rotary[i].previousValueB - controlRegister[MAPPING::ANALOG_HYSTERESIS]))  // check if new reading is lower (by <debounce value>), if so...
  {
