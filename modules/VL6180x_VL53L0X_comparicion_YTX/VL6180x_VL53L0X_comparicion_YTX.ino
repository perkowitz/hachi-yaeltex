...
uint32_t mode = 0;
uint32_t maxDistance = MAX_DISTANCE;
...
MovingAverage<uint32_t> *movingAverage[2];
...
void setup() 
{
  ...
  for(int i=0;i<2;i++)
  {
    movingAverage[i] = new MovingAverage<uint32_t> (AVERAGE_SIZE, 0);
  } 
  //
}
...
{ 
  ...
  for(int i=0;i<2;i++)
  {
    distance[i] = constrain(distance[i],MIN_DISTANCE,maxDistance);
    ...
    if(distance[i]!=antDistance[i])
    {
      ...
      {
        movingAverage[i]->push(distance[i]);
        distance[i] = movingAverage[i]->get();
      }  
      uint32_t ccValue = map(distance[i],MIN_DISTANCE,maxDistance,0,127);
...
void handleNoteOn(byte channel, byte note, byte velocity)
{
  if(note==0)
  {
    mode = constrain(velocity,0,2);
    SerialUSB.print("Modo: ");
    SerialUSB.println(mode);
  }
  else if(note==1)
  {
    uint32_t newMaxDistance = velocity*10;
    maxDistance = constrain(newMaxDistance,MIN_DISTANCE,MAX_DISTANCE);
    SerialUSB.print("Max distance: ");
    SerialUSB.println(maxDistance);
  }  
}