...
uint32_t PAGE_SIZE_SB, PAGES_SB, MAX_FLASH_SB;
      ...
      else if (command == 'X')
      {
        ...
        while (dst_addr < MAX_FLASH_SB)
        {
          ...
          while (NVMCTRL->INTFLAG.bit.READY == 0)
            ;
          dst_addr += PAGE_SIZE_SB * 4; // Skip a ROW
        }
      ...
      else if (command == 'Y')
      {
        ...
        else
        {
          ...
          while (size)
          {
            ...
            // Fill page buffer
            uint32_t i;
            for (i=0; i<(PAGE_SIZE_SB/4) && i<size; i++)
            {
...
void sam_ba_monitor_run(void)
{
	...
	PAGE_SIZE_SB = pageSizes[NVMCTRL->PARAM.bit.PSZ];
	PAGES_SB = NVMCTRL->PARAM.bit.NVMP;
	MAX_FLASH_SB = PAGE_SIZE_SB * PAGES_SB;
